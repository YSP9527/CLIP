...
#CUDA_VISIBLE_DEVICES=0 python 研究拓展/CLIP/train.py
...
def train():
    ...
    # 3. 准备 CLIP 文本特征
    text_tokens = clip.tokenize(cfg.PROMPTS).to(cfg.DEVICE)
    with torch.no_grad():
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        for images, labels in pbar_train:
            ...
            sim_logits = logit_scale.exp() * clip_image_features @ text_features.t()
        ...
        if epoch_val_acc > best_val_acc:
            ...
            save_path = os.path.join(cfg.SAVE_DIR, '(Acc: {best_val_acc:.2f}%)_','best_model.pth')
            torch.save({
                'epoch': epoch + 1,
