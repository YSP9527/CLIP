...
#CUDA_VISIBLE_DEVICES=0 python 研究拓展/CLIP/原始模型/ResNet50/train_baseline.py
# 尝试导入较新版本的插值模式，兼容老版本
try:
...
class Config:
    ...
    # 输出与日志路径
    SAVE_DIR = r'研究拓展/CLIP/原始模型/ResNet50/checkpoints'
    LOG_FILE = r'研究拓展/CLIP/原始模型/ResNet50/runs/baseline_resnet50.log'
...
def get_dataloaders(cfg):
    # 标准的 ImageNet 归一化参数，适用于 ResNet50
...
def main():
    ...
    logger.info("===========================================")
    logger.info(f"Starting ResNet50 Baseline Training on {cfg.DEVICE}")
    logger.info("===========================================")
    ...
    for epoch in range(cfg.EPOCHS):
        ...
        if epoch_val_acc > best_val_acc:
            ...
            save_path = os.path.join(cfg.SAVE_DIR, '(Acc: {best_val_acc:.2f}%)_','resnet50_baseline_best.pth')
            torch.save({
                'epoch': epoch + 1,
